...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your WaterKettleRestApi account.<br>
        ...
        &mdash; WaterKettleRestApi Team
      `
      return sendMail({ toEmail: email, subject: 'WaterKettleRestApi - Password Reset', content })
    })
